...
/**
 * Standalone PS2 Portfolio Debug Script
 *
 * This script connects to PS2 (Portfolio Management System) and fetches portfolio debug report.
 *
 * Usage:
 *   node debug_portfolio.js
 *
 * Configuration:
...
class PS2LoginHandler {
        ...
        return new Promise((resolve, reject) => {
            ...
            const handleResponse = (response) => {
                try {
                     ...
                     * Fragment 0/3: {"msgId":"msg_123","index":0,"total":3,"data":"{\"command\":\"portfolios.debug\",\"data\":["}
                     ...
                     *
                     * Combined JSON: {"command":"portfolios.debug","data":[{"symbol":"AAPL","volume":100},{"symbol":"GOOGL","volume":50}]}
                     */
                    ...
                    if (total > 1) {
                        ...
                        if (message.command === 'portfolios.debug') {
                            // Parse debug data
                            if (data && typeof data === 'string') {
                                try {
                                    const parsedData = JSON.parse(data);
                                    console.log('Parsed debug data from string');
                                    cleanup();
                                    resolve(parsedData);
                                } catch (e) {
                                    console.error('Failed to parse debug data string:', e);
                                    cleanup();
                                    reject(e);
                                }
                            } else if (Array.isArray(data)) {
                                console.log('Received debug data array directly');
                                cleanup();
                                resolve({ data });
                            } else {
                                console.log('Received other debug response format');
                                cleanup();
                                resolve(response);
                            }
                        } else {
    ...
    async fetchTrades(portfolioId) {
        ...
        const message = {
            command: 'trades.list',
            filter: { portfolioId },
            msgId: this.generateMsgId('trades')
        };
    ...
    async fetchDebugReport(portfolioId) {
        if (!this.token) {
            throw new Error('Not logged in. Call login() first');
        }
...
async function debugPortfolio() {
    const portfolioId = '6915feb01d51dd1539179a0c'; // Same as positions test
    ...
    try {
        ...
        // Fetch trades list first
        console.log(`Fetching trades for portfolio: ${portfolioId}`);
        const trades = await ps2Handler.fetchTrades(portfolioId);
        console.log('Trades list result:');
        console.log(JSON.stringify(trades, null, 2));

        // Fetch portfolio debug report
        console.log(`Fetching debug report for portfolio: ${portfolioId}`);
        const debugReport = await ps2Handler.fetchDebugReport(portfolioId);

        console.log('Portfolio debug report result:');
