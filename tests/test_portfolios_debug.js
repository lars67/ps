...
/**
 * PS2 Portfolios Debug Test Script
 *
 * This script tests the portfolios.debug command with different parameter formats.
 */
...
class PS2LoginHandler {
                    ...
                    if (total > 1) {
                        ...
                        if (receivedCount === expectedTotal) {
                            ...
                            try {
                                ...
                                console.log(`Parsed fragmented data:`, JSON.stringify(parsedData, null, 2));
                                cleanup();
                                resolve(parsedData);
                            } catch (error) {
                        ...
                        // Default response handling for non-fragmented messages
    ...
    async fetchDebug(portfolioId, fee, granularity, options = {}) {
        if (!this.token) {
        ...
        const message = {
            ...
            token: this.token,
            msgId: this.generateMsgId('debug')
        };

        console.log('Sending debug command:', JSON.stringify(message, null, 2));
...
async function testPortfoliosDebug() {
    ...
    try {
        ...
        // Test Command A: With args wrapper
        console.log('\n=== Test Command A: With args wrapper ===');
        try {
            const debugResultA = await ps2Handler.fetchDebug(portfolioId, 0, 'day', {
                from: '2025-10-01',
                till: '2025-10-16',
                exportToCsv: false,
                includeSummaries: false,
                fileName: 'my_portfolio_debug.csv'
            });

            console.log('Debug Result A:');
            console.log(JSON.stringify(debugResultA, null, 2));
        } catch (error) {
            console.error('Debug Test A failed:', error.message);
        }

        // Test Command B: Without args wrapper (should fail)
        console.log('\n=== Test Command B: Without args wrapper (expected to fail) ===');
        try {
            const messageB = {
                command: 'portfolios.debug',
                portfolioId,
            ...
            console.log('Sending raw command B:', JSON.stringify(messageB, null, 2));
            // Use the WebSocket directly for this test
            if (!ps2Handler.ws || ps2Handler.ws.readyState !== WebSocket.OPEN) {
                await ps2Handler.connectWebSocket('ps2', ps2Handler.token);
            }

            // Send directly without token/message wrapper
            ps2Handler.ws.send(JSON.stringify(messageB));
            console.log('Sent command B, listening for response...');

            // Wait a bit for response
            await new Promise(resolve => setTimeout(resolve, 5000));
        } catch (error) {
            console.error('Debug Test B failed:', error.message);
        }
