...
const createFeature = async ({symbols}:{symbols:string[]| string}, sendResponse: (data:any)=> void) => {
   ...
   const symbolsAr = typeof symbols === 'string' ? symbols.split(',') : symbols;
    ...
    const timerId =setInterval(()=> {
        data = data.map(d=> ({...d, bid: d.bid+Math.round(5*Math.random() -2)}))
        sendResponse(data);
    }, 3000)
    return {timerId,data} ;
}
...
export const subscribe = async (par:any,  sendResponse:  (data:object)=>void, msgId:string, userModif: string, userId:string) =>  {
...
export const unsubscribe = async (par:any, sendResponse:  (data:object)=>void, msgId:string, userModif: string, userId:string ) => {
  const {subscribeMsgId} =par;
  const fKey = getFKey(subscribeMsgId, userModif);
  console.log('subscribeMsgId', subscribeMsgId, 'fKey', fKey,Object.keys(features));

  if(features[fKey]) {
      clearInterval(features[fKey])
      console.log('UNSUBSCRIBE////////////////////// ', {subscribeMsgId}, 'fKey', fKey)
  }
...
export const description:CommandDescription ={
    ...
    unsubscribe: {
        ...
        value: JSON.stringify({command:'symbols.unsubscribe',subscribeMsgId:'?'})
    },
