...
export async function list(filter:FilterQuery<Currency>={}): Promise<Currency[]|null> {
    try {
...
export async function update(Currency:Partial<CurrencyWithID>): Promise<Currency|null>{
    const {_id, ...other} = Currency
    return await CurrencyModel.findByIdAndUpdate(_id, other);
}
