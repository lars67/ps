...
import SSEService, {QuoteData, SSEServiceInst} from "../../services/app/SSEService";
import eventEmitter from "../../services/app/eventEmiter";
...
export async function positions(
  { _id, requestType, subscribeId, marketPrice = "4", basePrice = "4" }: Params,
  ...
  const symbols = [
    ...
    ...extractUniqueFields(positions.positions, "currency").map(
      (c: string) => `${c}${portfolio.currency}:FX`,
    ),
  ].join(",");
  const sseService = new SSEService("quotes", `symbols=${symbols}`, eventName);
  subscribers[msgId] = prepareSubscriber(
    positions.positions,
    positions.fees,
    sseService,
    sendResponse,
  ...
  return { msg: requestType === "1" ? "subscribed" : "snapshot" };
}
...
function prepareSubscriber(
  ...
  const portfolioPositions: Record<
    string,
    Partial<PortfolioPositionFull>
  > = positions.reduce(
